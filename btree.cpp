...
bool test_btree() {
  ...
  if (handles1->empty()) {
    //cout << "HeapTable compare 1 - Not Found!!!" << endl;
    result = false;
  }
  else {
  ...
  if (handles2->empty()) {
    //cout << "HeapTable compare 2 - Not Found!!!" << endl;
    result = false;
  }
  else {
  ...
  cout << "HeapTable compare 4 - complex" << endl;
  result = false;
  for (uint j = 1; j < 1000; j++) {
    ...
    if (handles4->empty()) {
      cout << "HeapTable compare 4 - Not Found!!!" << endl;
      result = false;
    }
    else {
      for (auto const& handle : *handles4) {
        ...
        if ((*result_row)["a"] == test_row4["a"]) {
          //cout << "HeapTable compare 4 - Found!!!" << endl;
          result = true;
          break;
        }
        delete result_row;
